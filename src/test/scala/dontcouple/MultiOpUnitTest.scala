...
class MultiOpM extends Module {
  ...
  val recv_i = new MultiOp(recv_i1 :: recv_i2 :: Nil,
    on_done = () => {
...
class MultiOpUnitTester(m: MultiOpM) extends PeekPokeTester(m) {
...
class MultiOpTester extends ChiselFlatSpec with Dontcouple_Context with Dontcouple_CombFilter_Context {

  val mgen = () => new MultiOpM

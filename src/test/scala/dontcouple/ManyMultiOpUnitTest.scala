...
class ManyMultiOpM extends Module {
  ...
  val recv_i = new ManyMultiOp(recv_i1 :: recv_i2 :: Nil,
    on_done = () => {
      o_reg := i1_reg + 2.U * i2_reg
      state := sSend
      send_o.kick()
    }
  )
...
class ManyMultiOpUnitTester(m: ManyMultiOpM) extends PeekPokeTester(m) {
...
class ManyMultiOpTester extends ChiselFlatSpec with Dontcouple_Context with Dontcouple_CombFilter_Context {

  val mgen = () => new ManyMultiOpM
